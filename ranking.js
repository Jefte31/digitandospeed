(() => {
      ...
      <div class="ranking-panel" role="dialog" aria-modal="true" aria-labelledby="rankingTitle">
        <div class="ranking-head">
          <div>
            ...
            <p class="ranking-subtitle">Melhores partidas deste navegador, separadas por dificuldade e nível alcançado.</p>
          </div>
        ...
        <p class="ranking-note">Ranking local: os resultados ficam salvos somente neste navegador.</p>
      </div>`;
  ...
  function renderTable() {
    ...
    content.innerHTML = `<div class="ranking-table-wrap"><table class="ranking-table">
      <thead><tr><th>#</th><th>Nível</th><th>Pontos</th><th>PPM</th><th>Precisão</th></tr></thead>
      <tbody>${records.map((r, i) => `<tr><td>${i + 1}</td><td>Nível ${r.level}</td><td>${Number(r.score).toLocaleString("pt-BR")}</td><td>${r.ppm}</td><td>${r.accuracy}%</td></tr>`).join("")}</tbody>
    </table></div>`;
  }
