(() => {
  ...
  function togglePause(force) {
    ...
    if (shouldPause && state.mode === "playing") {
      state.mode = "paused"; ui.pauseScreen.classList.add("visible"); ui.pauseBtn.textContent = "▶";
    } else if (!shouldPause && state.mode === "paused") {
      state.mode = "playing"; ui.pauseScreen.classList.remove("visible"); ui.pauseBtn.textContent = "Ⅱ"; lastTime = performance.now();
    }
  }
  ...
  function wordPoolForLevel(level) {
    ...
    if (effective <= 2) return [...WORDS.easy, ...WORDS.medium.slice(0, 8)];
    if (effective <= 5) return [...WORDS.easy, ...WORDS.medium];
    if (effective <= 8) return [...WORDS.medium, ...WORDS.hard];
    if (effective <= 12) return [...WORDS.medium.slice(8), ...WORDS.hard, ...WORDS.expert.slice(0, 8)];
  ...
  function damage() {
    state.lives -= 1; state.streak = 0; state.target = null; state.shake = 12; state.flash = 0.34;
    sfx("damage"); updateHud();
  ...
  function destroyEnemy(enemy) {
    ...
    sfx("destroy");
    if (state.level > state.lastLevel) { state.lastLevel = state.level; showToast(`NÍVEL ${state.level}`); sfx("level"); }
    updateHud();
  }
  ...
  function explosion(x, y, radius) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      particles.push({ x, y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, life: 0.35 + Math.random() * 0.55, maxLife: 0.9, size: 1 + Math.random() * 3, hue: Math.random() > 0.35 ? 188 : 257 });
    }
  }
  ...
  function fireLaser(enemy) {
    ...
    lasers.push({ x1: ship.x, y1: ship.y - 19, x2: enemy.x, y2: enemy.y, life: 0.13, maxLife: 0.13 });
    enemy.hit = 0.15; enemy.pulse = 0.2; sfx("type");
  }
  ...
  function processKey(rawKey) {
    ...
    if (!state.target || !enemies.includes(state.target)) {
      const candidates = enemies.filter((e) => [...e.word][0].toLocaleLowerCase("pt-BR") === key).sort((a, b) => b.y - a.y);
    ...
    if (target) {
      ...
      if (key === expected) {
        target.typed += 1; state.correctChars += 1; state.streak += 1;
      ...
      if (key === firstLetter) { target.typed = 1; state.correctChars += 1; fireLaser(target); }
      state.shake = Math.max(state.shake, 2.5);
      showToast("ERRO — REDIGITE A PALAVRA"); sfx("error"); updateHud(); return;
    }

    state.streak = 0; state.shake = Math.max(state.shake, 2.5); sfx("error"); updateHud();
  }

  function update(dt) {
    updateStars(dt); updateEffects(dt);
    if (state.mode !== "playing") return;
    state.elapsed += dt; state.spawnClock += dt;
    if (state.spawnClock >= spawnInterval()) { state.spawnClock = 0; spawnEnemy(); }
    for (const enemy of [...enemies]) {
      enemy.phase += dt * 1.35; enemy.rot += enemy.rotSpeed * dt; enemy.y += enemy.speed * dt;
      enemy.x += Math.sin(enemy.phase) * 5.5 * dt; enemy.hit = Math.max(0, enemy.hit - dt); enemy.pulse = Math.max(0, enemy.pulse - dt);
      if (enemy.y > height - 64) {
        explosion(enemy.x, height - 66, enemy.radius); enemies.splice(enemies.indexOf(enemy), 1);
        if (state.target === enemy) state.target = null;
        damage(); if (state.mode !== "playing") break;
      }
    }
  ...
  function updateStars(dt) {
    ...
    for (const star of stars) { star.y += star.speed * multiplier * dt; if (star.y > height + 3) { star.y = -3; star.x = Math.random() * width; } }
  }

  function updateEffects(dt) {
    lasers = lasers.filter((l) => (l.life -= dt) > 0);
    particles = particles.filter((p) => { p.life -= dt; p.x += p.vx * dt; p.y += p.vy * dt; p.vx *= Math.pow(0.985, dt * 60); p.vy *= Math.pow(0.985, dt * 60); return p.life > 0; });
    shockwaves = shockwaves.filter((s) => { s.r += 115 * dt; s.alpha -= 1.8 * dt; return s.alpha > 0; });
  }

  function draw() {
    ctx.save(); ctx.clearRect(0, 0, width, height);
    ctx.translate(state.shake ? (Math.random() - 0.5) * state.shake : 0, state.shake ? (Math.random() - 0.5) * state.shake : 0);
    drawBackground(); drawStars(); drawLane();
    for (const enemy of enemies) drawEnemy(enemy);
    drawLasers(); drawParticles(); drawShip(); drawTargetLink();
    if (state.flash > 0) { ctx.fillStyle = `rgba(251,113,133,${Math.min(0.12, state.flash * 0.35)})`; ctx.fillRect(-20, -20, width + 40, height + 40); }
    ctx.restore();
  }

  function drawBackground() {
    const gradient = ctx.createRadialGradient(width / 2, height * 0.72, 0, width / 2, height * 0.72, Math.max(width, height) * 0.72);
    gradient.addColorStop(0, "rgba(11,39,78,.16)"); gradient.addColorStop(0.48, "rgba(6,15,39,.1)"); gradient.addColorStop(1, "rgba(2,5,16,0)");
    ctx.fillStyle = gradient; ctx.fillRect(0, 0, width, height);
  }

  function drawStars() {
    for (const star of stars) { ctx.globalAlpha = star.alpha; ctx.fillStyle = star.layer > 0.84 ? "#b8f4ff" : "#93a7bf"; ctx.fillRect(star.x, star.y, star.size, star.size * (1 + star.layer * 0.8)); }
    ctx.globalAlpha = 1;
  }

  function drawLane() {
    const ship = shipPosition(); ctx.save(); ctx.strokeStyle = "rgba(103,232,249,.035)"; ctx.lineWidth = 1; ctx.setLineDash([2, 12]);
    ctx.beginPath(); ctx.moveTo(ship.x, 95); ctx.lineTo(ship.x, ship.y - 40); ctx.stroke(); ctx.restore();
  }

  function drawEnemy(enemy) {
    const isTarget = state.target === enemy; ctx.save(); ctx.translate(enemy.x, enemy.y); ctx.rotate(enemy.rot);
    if (isTarget) {
      const halo = ctx.createRadialGradient(0, 0, 3, 0, 0, enemy.radius * 3.1); halo.addColorStop(0, "rgba(103,232,249,.17)"); halo.addColorStop(1, "rgba(103,232,249,0)");
      ctx.fillStyle = halo; ctx.beginPath(); ctx.arc(0, 0, enemy.radius * 3.1, 0, TAU); ctx.fill();
    }
    ctx.rotate(-enemy.rot); ctx.shadowBlur = enemy.hit > 0 ? 22 : (isTarget ? 14 : 5); ctx.shadowColor = isTarget ? "rgba(103,232,249,.9)" : "rgba(111,129,160,.35)";
    ctx.fillStyle = enemy.hit > 0 ? "rgba(191,248,255,.98)" : "rgba(15,27,51,.96)"; ctx.strokeStyle = isTarget ? "rgba(103,232,249,.78)" : "rgba(129,152,184,.28)"; ctx.lineWidth = isTarget ? 1.4 : 1;
    ctx.beginPath();
    for (let i = 0; i < 6; i++) { const a = (i / 6) * TAU - Math.PI / 2, r = enemy.radius * (i % 2 ? 0.86 : 1), px = Math.cos(a) * r, py = Math.sin(a) * r; i ? ctx.lineTo(px, py) : ctx.moveTo(px, py); }
    ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.shadowBlur = 0; ctx.fillStyle = isTarget ? "rgba(103,232,249,.8)" : "rgba(133,151,176,.5)"; ctx.beginPath(); ctx.arc(0, 0, 2.4, 0, TAU); ctx.fill(); ctx.restore();
    drawWord(enemy, isTarget);
  }

  function drawWord(enemy, isTarget) {
    const chars = [...enemy.word], typed = chars.slice(0, enemy.typed).join(""), rest = chars.slice(enemy.typed).join("");
    const fontSize = Math.max(14, Math.min(18, width / 75)); ctx.save(); ctx.font = `800 ${fontSize}px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace`; ctx.textBaseline = "middle";
    const typedW = ctx.measureText(typed).width, restW = ctx.measureText(rest).width, totalW = typedW + restW, x = enemy.x - totalW / 2, y = enemy.y - enemy.radius - 20;
    if (isTarget) { ctx.fillStyle = "rgba(4,14,31,.74)"; roundRect(ctx, x - 9, y - fontSize / 2 - 6, totalW + 18, fontSize + 12, 7); ctx.fill(); ctx.strokeStyle = "rgba(103,232,249,.12)"; ctx.stroke(); }
    ctx.fillStyle = isTarget ? "#62dfee" : "rgba(121,143,168,.7)"; ctx.fillText(typed, x, y); ctx.fillStyle = isTarget ? "#edfaff" : "#c4d0dd"; ctx.fillText(rest, x + typedW, y); ctx.restore();
  }
  ...
  function drawShip() {
    const { x, y } = shipPosition(); ctx.save(); ctx.translate(x, y);
    const engine = ctx.createRadialGradient(0, 19, 0, 0, 19, 48); engine.addColorStop(0, "rgba(70,205,255,.34)"); engine.addColorStop(1, "rgba(70,205,255,0)"); ctx.fillStyle = engine; ctx.beginPath(); ctx.arc(0, 18, 48, 0, TAU); ctx.fill();
    ctx.shadowBlur = 24; ctx.shadowColor = "rgba(90,222,255,.34)"; ctx.fillStyle = "#d8f8ff"; ctx.strokeStyle = "rgba(103,232,249,.8)"; ctx.lineWidth = 1.4;
    ...
    ctx.shadowBlur = 0; ctx.fillStyle = "#09233b"; ctx.beginPath(); ctx.moveTo(0, -16); ctx.lineTo(6, 8); ctx.lineTo(0, 13); ctx.lineTo(-6, 8); ctx.closePath(); ctx.fill();
    ctx.fillStyle = "#67e8f9"; ctx.fillRect(-5, 20, 3, 7); ctx.fillRect(2, 20, 3, 7); ctx.restore();
  }

  function drawTargetLink() {
    if (!state.target || state.mode !== "playing") return;
    const ship = shipPosition(); ctx.save(); ctx.strokeStyle = "rgba(103,232,249,.055)"; ctx.setLineDash([3, 8]); ctx.beginPath(); ctx.moveTo(ship.x, ship.y - 30); ctx.lineTo(state.target.x, state.target.y + 16); ctx.stroke(); ctx.restore();
  }

  function drawLasers() {
    for (const l of lasers) { ctx.save(); ctx.globalAlpha = Math.max(0, l.life / l.maxLife); ctx.strokeStyle = "#78efff"; ctx.shadowBlur = 13; ctx.shadowColor = "#67e8f9"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(l.x1, l.y1); ctx.lineTo(l.x2, l.y2); ctx.stroke(); ctx.restore(); }
  }

  function drawParticles() {
    for (const p of particles) { const alpha = Math.max(0, Math.min(1, p.life / p.maxLife)); ctx.fillStyle = `hsla(${p.hue},92%,70%,${alpha})`; ctx.fillRect(p.x, p.y, p.size, p.size); }
    for (const s of shockwaves) { ctx.save(); ctx.globalAlpha = s.alpha; ctx.strokeStyle = "#79ecff"; ctx.lineWidth = 1.4; ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, TAU); ctx.stroke(); ctx.restore(); }
  }

  function loop(now) { const dt = Math.min((now - lastTime) / 1000, 0.05); lastTime = now; update(dt); draw(); animationId = requestAnimationFrame(loop); }
  ...
  function tone(freq, duration, volume = 0.035, type = "sine", slide = 0) {
    if (muted) return; ensureAudio(); const t = audio.currentTime, osc = audio.createOscillator(), gain = audio.createGain(); osc.type = type; osc.frequency.setValueAtTime(freq, t);
    if (slide) osc.frequency.exponentialRampToValueAtTime(Math.max(30, freq + slide), t + duration); gain.gain.setValueAtTime(volume, t); gain.gain.exponentialRampToValueAtTime(0.0001, t + duration); osc.connect(gain); gain.connect(audio.destination); osc.start(t); osc.stop(t + duration);
  }
  function sfx(name) {
    if (muted) return;
    try {
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Backspace" && state.mode === "playing" && state.target) {
      event.preventDefault(); state.target.typed = 0; state.streak = 0; showToast("PALAVRA REINICIADA"); updateHud(); return;
    }
    if (event.key.length === 1) { event.preventDefault(); processKey(event.key); }
  }, { passive: false });
  ...
  updateDifficultyUi(); resize(); updateHud(); cancelAnimationFrame(animationId); animationId = requestAnimationFrame(loop);
})();
